...
export function PublicBookingLink({ slug, enabled = true }: { slug: string; enabled?: boolean }) {
  ...
  if (!enabled) return <span className="text-xs text-slate-400">Available after publishing</span>;

  return (
    <div className="flex flex-wrap items-center gap-2">
      <code className="min-w-0 max-w-full truncate rounded-[4px] bg-slate-100 px-2 py-1 text-xs text-slate-600">{path}</code>
      <Button
        type="button"
        variant="secondary"
